...
func Memcache_projects_locations_instances_applysoftwareupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ApplySoftwareUpdateRequest
		...
		url := fmt.Sprintf("%s/v1beta2/%s:applySoftwareUpdate%s", cfg.BaseURL, instance, queryString)
...
func CreateMemcache_projects_locations_instances_applysoftwareupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1beta2_instance_applySoftwareUpdate",
		mcp.WithDescription("Updates software on the selected nodes of the Instance."),
		...
		mcp.WithString("instance", mcp.Required(), mcp.Description("Required. Resource name of the Memcached instance for which software update should be applied.")),
		mcp.WithBoolean("applyAll", mcp.Description("Input parameter: Whether to apply the update to all nodes. If set to true, will explicitly restrict users from specifying any nodes, and apply software update to all nodes (where applicable) within the instance.")),
		mcp.WithArray("nodeIds", mcp.Description("Input parameter: Nodes to which we should apply the update to. Note all the selected nodes are updated in parallel.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Memcache_projects_locations_instances_applysoftwareupdateHandler(cfg),
	}
}
