...
func Memcache_projects_locations_operations_cancelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CancelOperationRequest
		...
		url := fmt.Sprintf("%s/v1beta2/%s:cancel%s", cfg.BaseURL, name, queryString)
...
func CreateMemcache_projects_locations_operations_cancelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1beta2_name_cancel",
		mcp.WithDescription("Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. Clients can use Operations.GetOperation or other methods to check whether the cancellation succeeded or whether the operation completed despite cancellation. On successful cancellation, the operation is not deleted; instead, it becomes an operation with an Operation.error value with a google.rpc.Status.code of 1, corresponding to `Code.CANCELLED`."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("The name of the operation resource to be cancelled.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Memcache_projects_locations_operations_cancelHandler(cfg),
	}
}
