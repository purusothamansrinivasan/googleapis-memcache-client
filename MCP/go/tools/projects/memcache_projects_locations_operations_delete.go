...
func Memcache_projects_locations_operations_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1beta2/%s%s", cfg.BaseURL, name, queryString)
...
func CreateMemcache_projects_locations_operations_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v1beta2_name",
		mcp.WithDescription("Deletes a long-running operation. This method indicates that the client is no longer interested in the operation result. It does not cancel the operation. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("The name of the operation resource to be deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Memcache_projects_locations_operations_deleteHandler(cfg),
	}
}
