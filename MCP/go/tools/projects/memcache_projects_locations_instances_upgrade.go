...
func Memcache_projects_locations_instances_upgradeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.GoogleCloudMemcacheV1beta2UpgradeInstanceRequest
		...
		url := fmt.Sprintf("%s/v1beta2/%s:upgrade%s", cfg.BaseURL, name, queryString)
...
func CreateMemcache_projects_locations_instances_upgradeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1beta2_name_upgrade",
		mcp.WithDescription("Upgrades the Memcache instance to a newer memcached engine version specified in the request."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Required. Memcache instance resource name using the form: `projects/{project}/locations/{location}/instances/{instance}` where `location_id` refers to a GCP region.")),
		mcp.WithString("memcacheVersion", mcp.Description("Input parameter: Required. Specifies the target version of memcached engine to upgrade to.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Memcache_projects_locations_instances_upgradeHandler(cfg),
	}
}
