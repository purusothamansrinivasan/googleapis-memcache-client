...
func Memcache_projects_locations_operations_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateMemcache_projects_locations_operations_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1beta2_name",
		mcp.WithDescription("Gets the latest state of a long-running operation. Clients can use this method to poll the operation result at intervals as recommended by the API service."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("The name of the operation resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Memcache_projects_locations_operations_getHandler(cfg),
	}
}
