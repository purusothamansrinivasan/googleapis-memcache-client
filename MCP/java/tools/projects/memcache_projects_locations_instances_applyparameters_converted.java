/**
 * MCP Server function for `ApplyParameters` restarts the set of specified nodes in order to update them to the current set of parameters for the Memcached Instance.
 */
...
class Post_V1Beta2_Name_Apply_ParametersMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_V1Beta2_Name_Apply_ParametersHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("nodeIds")) {
            queryParams.add("nodeIds=" + args.get("nodeIds"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_v1beta2_name_apply_parameters" + queryString;
    ...
    public static MCPServer.Tool createPost_V1Beta2_Name_Apply_ParametersTool(MCPServer.APIConfig config) {
        ...
        nameProperty.put("description", "Required. Resource name of the Memcached instance for which parameter group updates should be applied.");
        ...
        applyAllProperty.put("description", "Input parameter: Whether to apply instance-level parameter group to all nodes. If set to true, users are restricted from specifying individual nodes, and `ApplyParameters` updates all nodes within the instance.");
        ...
        nodeIdsProperty.put("description", "Input parameter: Nodes to which the instance-level parameter group is applied.");
        properties.put("nodeIds", nodeIdsProperty);
        parameters.put("properties", properties);
        
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_v1beta2_name_apply_parameters",
            "`ApplyParameters` restarts the set of specified nodes in order to update them to the current set of parameters for the Memcached Instance.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_V1Beta2_Name_Apply_ParametersHandler(config));
    }
