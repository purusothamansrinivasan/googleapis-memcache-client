...
class Post_V1Beta2_Name_CancelMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_V1Beta2_Name_CancelHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_v1beta2_name_cancel" + queryString;
    ...
    public static MCPServer.Tool createPost_V1Beta2_Name_CancelTool(MCPServer.APIConfig config) {
        ...
        nameProperty.put("description", "The name of the operation resource to be cancelled.");
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_v1beta2_name_cancel",
            "Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. Clients can use Operations.GetOperation or other methods to check whether the cancellation succeeded or whether the operation completed despite cancellation. On successful cancellation, the operation is not deleted; instead, it becomes an operation with an Operation.error value with a google.rpc.Status.code of 1, corresponding to `Code.CANCELLED`.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_V1Beta2_Name_CancelHandler(config));
    }
