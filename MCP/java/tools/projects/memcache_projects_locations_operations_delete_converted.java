/**
 * MCP Server function for Deletes a long-running operation. This method indicates that the client is no longer interested in the operation result. It does not cancel the operation. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`.
 */
...
class Delete_V1Beta2_NameMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getDelete_V1Beta2_NameHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/delete_v1beta2_name" + queryString;
    ...
    public static MCPServer.Tool createDelete_V1Beta2_NameTool(MCPServer.APIConfig config) {
        ...
        nameProperty.put("description", "The name of the operation resource to be deleted.");
        ...
        return new MCPServer.Tool(definition, getDelete_V1Beta2_NameHandler(config));
    }
